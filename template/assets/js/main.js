...
$('#form-footer').on('submit',function () {
    var forma=$('#form-footer').serialize();
    ...
    $.ajax({
        ...
        data:   forma ,
        success: function(data){
            $('#form-footer').html('Спасибо за обращение.')
    ...
    $('#form-page').on('submit',function () {
        ...
        $.ajax({
            ...
            data:   forma ,
            success: function(data){
                $('#form-page').html('<p class="section-title">Спасибо за обращение.</p>');
				NeirosEventSend('test-event',
					{"type": 'form',
						"data": {
							"name": 	data.name,
    ...
    $('#xs-contact-form').on('submit',function () {
        var forma=$('#xs-contact-form').serialize();
        ...
        $.ajax({
            ...
            data:   forma ,
            success: function(data){
                $('#xs-contact-form').html('<p class="section-title">Спасибо за обращение.</p>');
				NeirosEventSend('test-event',
					{"type": 'form',
						"data": {
							"name":data.name,
...
jQuery(window).scroll(function(){
	...
	$sections.each(function(i,el){
    	...
    	if( scroll > top && scroll < bottom){
            $('a.active-color-link').removeClass('active-color-link');
			$('a[href="#'+id+'"]').addClass('active-color-link');
...
$(document).on('click','.btnpodpisatisya',function (){

	$.ajax({
		method:'get',
		url:'https://neiros.ru/ajax/subscribe/',
		data:{
			email:$('#newsletter-thumb').val()
		},success:function (){
		$('.mess1').html('Вы успешно подписаны')
			$('#newsletter-thumb').val('')
		}
